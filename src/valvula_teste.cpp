...
class ImageConverter
{
    ...
    void valveCallback(const sensor_msgs::ImageConstPtr &msg)
    {
        ...
        if(contours_selected.size() > 0)
        {
            ...
            float rel_hw = -1;
            int minimum_area = 0.015 * (src.rows * src.cols);
            int maximum_area = 0.10 * (src.rows * src.cols);
            int area = rect_selected[i_max].width * rect_selected[i_max].height;
...
bool le_valvula(semear_ptr::Valvula::Request &req,
                semear_ptr::Valvula::Response &res)
{
    ImageConverter ic;
    ...
    res.esta_aberta = ic.esta_aberta_;
